...
public class PobMailDb extends CBO_DB_Helper {
    ...
    public static PobMailDb getInstance(Context context,Boolean ReInitiate) {
        if (instance == null || ReInitiate) {
        ...
        if (sd.isReadOnly()){
            instance = new PobMailDb(MyCustumApplication.getInstance());
            sd = getWritableDatabase();
        }
        return sd;
    }
    ...
    public ArrayList<mChemist> getChemist() {
        return getChemist(null);
    }
    public ArrayList<mChemist> getChemist(mChemist chemist1) {
        ArrayList<mChemist> chemists = new ArrayList<>();

        String whereClause = "WHERE CAST(dc.pob_amt as float) > 0 ";
        if (chemist1 != null){
            whereClause = "WHERE c.chem_id ='"+chemist1.getID()+"' AND CASE WHEN IFNull(p.CHEMIST_ID,0) >0 THEN 1 ELSE 0 END = 1 ";
        }
        ...
        try {
            c = getDatabase().rawQuery("select c.chem_id,c.chem_name," +
                    "dc.allitemid as item_id,dc.allitemqty as qty,dc.rate,"+
                    ...
                    " LEFT OUTER JOIN phdcrchem as dc ON dc.chem_id=c.chem_id "+
                    whereClause, null);
    ...
    public void deletePobMail(mChemist chemist){
        String whereClause = "";
        if (chemist != null){
            whereClause = "CHEMIST_ID ='"+chemist.getID()+"'";
        }

        getDatabase().delete(PobMailDb_TABLE_NAME,whereClause,null);
