...
import com.cbo.cbomobilereporting.ui_new.utilities_activities.newVisualaddDownload.MDownLoadVisualAdd;
...
import utils_new.cboUtils.attachmentsyncdr.mAttachmentSyncDr;
...
public class CBO_DB_Helper extends SQLiteOpenHelper {
    ...
    private static final String PH_Retailer = "PHRetailer";
    private static final String VISUAL_ADD_DOWNLOAD = "VADownload";
...
//        String PH_ITEM = "CREATE TABLE " + DOCTOR_PRODUCTS_TABLE + "( id integer primary key,item_id text,item_name text,stk_rate double,gift_type text,SHOW_ON_TOP text,SHOW_YN text,SPL_ID integer,GENERIC_NAME text,chem_rate double,dr_rate double,CAMPAIGN text)";
        //priyesh
        String PH_ITEM = "CREATE TABLE " + DOCTOR_PRODUCTS_TABLE + "( id integer primary key,item_id text,item_name text,stk_rate double,gift_type text,SHOW_ON_TOP text,SHOW_YN text,SPL_ID integer,GENERIC_NAME text,chem_rate double,dr_rate double,CAMPAIGN text,BRAND_ID text,BRAND_NAME text)";
        ...
        //priyesh
        String PH_Retailar = "CREATE TABLE " + PH_Retailer + "(id integer primary key autoincrement,custmerName text,mr_id text,route text,mobile text,concernPerson text,address1 text,address2 text,address3 text,rout_id text,pincode text,email text,visitFreequency text,category text,dateOfBirth text,marriageAnniversary text)";
        String VA_Download = "CREATE TABLE " + VISUAL_ADD_DOWNLOAD + "(id integer primary key autoincrement,itemId text,itemName text,fileName text,downLoadVersion text)";
        ...
        String tempdr = "CREATE TABLE tempdr ( id integer primary key,dr_id text,dr_name text,visit_time text,batteryLevel text,dr_latLong text,dr_address text,dr_remark text,updated text,dr_km text,srno text,work_with_name text,DR_AREA text,file text,call_type text,LOC_EXTRA text,Ref_latlong text,file_updated text)";
        ...
        String CREATE_TABLE_Lat_Long_Reg = "CREATE TABLE " + Lat_Long_Reg + "(id Integer PRIMARY KEY ,DCS_ID text,LAT_LONG text,DCS_TYPE text,DCS_ADD text,DCS_INDES text,UPDATED text,file_updated text,file text)";
        ...
        //priyesh
        db.execSQL(PH_Retailar);
        db.execSQL(VA_Download);
        db.execSQL(myRepa);
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        switch (oldVersion) {
            ...
            case 49:
                db.execSQL(PobMailDb.PobMailDb_Query);
            case 50:
                db.execSQL("ALTER TABLE " + "tempdr" + " ADD COLUMN file_updated text DEFAULT '1'");
            case 51:
                db.execSQL("ALTER TABLE " + Lat_Long_Reg + " ADD COLUMN file_updated text DEFAULT '1'");
                db.execSQL("ALTER TABLE " + Lat_Long_Reg + " ADD COLUMN file text DEFAULT ''");
            case 52:
                db.execSQL("ALTER TABLE " + DOCTOR_PRODUCTS_TABLE + " ADD COLUMN BRAND_ID text DEFAULT '1'");
                db.execSQL("ALTER TABLE " + DOCTOR_PRODUCTS_TABLE + " ADD COLUMN BRAND_NAME text DEFAULT ''");
//            case 53:
//                db.execSQL(VISUAL_ADD_DOWNLOAD);
////                db.execSQL("DROP TABLE IF EXISTS " +VISUAL_ADD_DOWNLOAD);
...
//=========================================================================phdoctor===============================================================================
    ...
    public void updateLatLong(String latlong, String id, String type, String index) {
        ...
        updateLatLong(latlong, id, type, index, "", "1", "0");

    }

    public void updateLatLong(String latlong, String id, String type, String index, String file, String fileUpdated) {

        updateLatLong(latlong, id, type, index, file, fileUpdated, "0");

    }


    public void updateLatLong(String latlong, String id, String type, String index, String file, String fileUpdated, String updated) {
        ...
        insertLat_Long_Reg(id, latlong, type, "", index, file, fileUpdated, updated);
...
//============================================================Add Chemist Table================================================================================
    ...
    public void updateDrFileUpdated(String drid, String file_updated) {
        try {
            sd = this.getWritableDatabase();
            ContentValues cv = new ContentValues();
            cv.put("file_updated", file_updated);
    ...
    public void updateDrFile(String drid, String file) {
        try {
            sd = this.getWritableDatabase();
            ContentValues cv = new ContentValues();
            cv.put("file", file);
    ...
    public ArrayList<mAttachmentSync> tempDrListFileUpdatePending(String file_updated) {
        ArrayList<mAttachmentSync> chem_list = new ArrayList<mAttachmentSync>();
        sd = this.getWritableDatabase();
        Cursor c = null;
        if (file_updated == null) {
            c = sd.rawQuery("select * from tempdr", null);
        } else {
            c = sd.rawQuery("select * from tempdr where file_updated ='" + file_updated + "'", null);
        }
        try {
            if (c.moveToFirst()) {
                do {
                    mAttachmentSync chemistList = new mAttachmentSync();
                    chemistList.setID(c.getString(c.getColumnIndex("dr_id")));
                    chemistList.setFILE_ATTACHMENT(c.getString(c.getColumnIndex("file")));
                    chemistList.setUPLOAD_STATUS(c.getInt(c.getColumnIndex("file_updated")) == 1);
                    chemistList.setNAME(c.getString(c.getColumnIndex("dr_name")));
                    chem_list.add(chemistList);
                } while (c.moveToNext());
            }
        } finally {
            c.close();
    ...
    public ArrayList<mAttachmentSyncDr> tempDrListAttachedFileUpdatePending(String file_updated) {

        ArrayList<mAttachmentSyncDr> chem_list = new ArrayList<mAttachmentSyncDr>();
        sd = this.getWritableDatabase();
        Cursor c = null;
        if (file_updated == null) {
            c = sd.rawQuery("select * from Lat_Long_Reg", null);
        } else {
            c = sd.rawQuery("select * from Lat_Long_Reg where file_updated ='" + file_updated + "'", null);
        }
        try {
            if (c.moveToFirst()) {
                do {
                    mAttachmentSyncDr chemistList = new mAttachmentSyncDr();
                    chemistList.setID(c.getString(c.getColumnIndex("DCS_ID")));
                    chemistList.setLAT_LONG(c.getString(c.getColumnIndex("LAT_LONG")));
                    chemistList.setDCS_TYPE(c.getString(c.getColumnIndex("DCS_TYPE")));
                    chemistList.setDCS_ADD(c.getString(c.getColumnIndex("DCS_ADD")));
                    chemistList.setDCS_INDES(c.getString(c.getColumnIndex("DCS_INDES")));
                    chemistList.setUPDATED(c.getString(c.getColumnIndex("UPDATED")));
                    chemistList.setFILE_ATTACHMENT(c.getString(c.getColumnIndex("file")));
                    chemistList.setUPLOAD_STATUS(c.getInt(c.getColumnIndex("file_updated")) == 1);
                    chemistList.setNAME("");
                    chem_list.add(chemistList);
        ...
        try {
            if (c.moveToFirst()) {
                do {
                    ...
                    //dcrid = dcrid.substring(dcrid.lastIndexOf("/")+1);
                    //Log.d("javed SRNO table",dcrid);
                } while (c.moveToNext());
            }
        } finally {
...
//   "CREATE TABLE " + MenuControl + "(id Integer PRIMARY KEY AUTOINCREMENT,menu text,menu_code text,menu_name text)";
  ...
  /*  public void insertLat_Long_Reg(String DCS_ID, String LAT_LONG,
                                   String DCS_TYPE, String DCS_ADD,
                                   String DCS_INDES, String file,
                                   String fileUpdated) {
        insertLat_Long_Reg(DCS_ID,LAT_LONG,DCS_TYPE,DCS_ADD,DCS_INDES,file,fileUpdated,"0");
    }
*/
    ...
    public void insertLat_Long_Reg(String DCS_ID, String LAT_LONG,
                                   String DCS_TYPE, String DCS_ADD,
                                   String DCS_INDES, String file,
                                   String fileUpdated, String updated) {
        try {
            sd = this.getWritableDatabase();
            ContentValues cv = new ContentValues();
            ...
            cv.put("UPDATED", updated);
            cv.put("file_updated", fileUpdated);
            cv.put("file", file);
    ...
    public void updatedLat_Long_RegFile(String DCS_ID) {
        try {
            sd = this.getWritableDatabase();
            ContentValues cv = new ContentValues();
            cv.put("file_updated", "1");
            int result = sd.update(Lat_Long_Reg, cv, "DCS_ID ='" + DCS_ID + "'", null);
        } finally {
            sd.close();
        }
...
// person_name text,person_id text,pob_amt text,allitemid text,allitemqty text,sample text,allgiftid text,allgiftqty text,file text,LOC_EXTRA text)";
    ...
    public Cursor getphitemSplByBrand() {
        sd = this.getWritableDatabase();
        //Cursor c=sd.rawQuery("select phitem.item_name,phitem.item_id from phitemspl inner join phitem phitem on phitem.item_id=phitemspl.item_id where phitemspl.dr_spl_id="+MyConnection.DOCTOR_SPL_ID+" order by phitem.item_name", null);
        return sd.rawQuery("select phitem.BRAND_ID,phitem.BRAND_NAME,phitem.item_name,phitem.item_id from phitemspl inner join phitem phitem on phitem.item_id=phitemspl.item_id where phitemspl.dr_spl_id=" + Custom_Variables_And_Method.DOCTOR_SPL_ID + " and SHOW_YN = '1' GROUP BY phitem.BRAND_ID order by phitemspl.srno", null);

    }

    public Cursor getSelectedFromDrByBrand(String dr_id) {
        try {
            sd = this.getWritableDatabase();
            return sd.rawQuery("select I.BRAND_ID,I.BRAND_NAME, D.item_name, D.item_id from phdoctoritem D LEFT JOIN phitem I on I.item_id = D.item_id where  SHOW_YN = '1' and dr_id=" + dr_id + " GROUP BY I.BRAND_ID", null);
        } finally {
            //sd.close();
        }
    }

    public Cursor getAllVisualAddByBrand(String itemidnotin, String SHOW_ON_TOP) {
        try {
            sd = this.getWritableDatabase();
            return sd.rawQuery("select min( ID) as G_ID,BRAND_ID,BRAND_NAME," +
                    " item_id, item_name,stk_rate from phitem where " +
                    "(gift_type='ORIGINAL' or  gift_type='OTHER') " +
                    "and SHOW_YN = '1' and SHOW_ON_TOP ='" + SHOW_ON_TOP + "' " +
                    "and item_id not in(" + (itemidnotin) + ") " +
                    "GROUP BY BRAND_ID" +
                    " ORDER BY G_ID", null);
        } finally {
            //sd.close();
    ...
    //=============================================================Doctor Products priyesh=======================================================================================
    public long insertProductsNew(String id, String name, double stk_rate, double chem_rate, double dr_rate, String gift,
                                  String SHOW_ON_TOP, String SHOW_YN, int SPL_ID, String GENERIC_NAME, String CAMPAIGN, String BRAND_ID, String BRAND_NAME) {
    ...
    public Cursor getAllItemsForGroup(String groupId) {


        try {
            sd = this.getWritableDatabase();
//            return sd.rawQuery("select BRAND_ID,BRAND_NAME, item_id, item_name,stk_rate from phitem where (gift_type='ORIGINAL' or  gift_type='OTHER') and SHOW_YN = '1' and SHOW_ON_TOP ='" + SHOW_ON_TOP + "' and item_id not in(" + (itemidnotin) + ") GROUP BY BRAND_ID", null);
//            return sd.rawQuery("select BRAND_ID,BRAND_NAME, item_name, item_id from phitem where SHOW_YN = '1' and BRAND_ID = " + groupId + " GROUP BY BRAND_ID", null);
            return sd.rawQuery("select BRAND_ID,BRAND_NAME, item_name, item_id from phitem where SHOW_YN = '1' and BRAND_ID = " + groupId, null);
    ...
    public ArrayList<String> getDoctorVisualIdByBrand(String dr_id) {

        ArrayList<String> doclist = new ArrayList<>();
        sd = this.getWritableDatabase();
        Cursor c = sd.rawQuery("select I.BRAND_ID from " + DOCTOR_ITEM_TABLE + " D LEFT JOIN phitem I on I.item_id = D.item_id where dr_id=" + dr_id + " and visual=1 GROUP BY I.BRAND_ID", null);

        try {
            if (c.moveToFirst()) {
                do {
                    doclist.add(c.getString(c.getColumnIndex("BRAND_ID")));
                } while (c.moveToNext());
            }
        } finally {
            c.close();
...
//    WHERE col <> ''  group by id) T on T.id= phitem.item_id
    ...
    public long SaveRetailer(MRetailerAdd chemistRegistration) {
        sd = this.getWritableDatabase();
        try {
            ContentValues contentValues = new ContentValues();
            contentValues.put("custmerName", chemistRegistration.getCustmerName());
            contentValues.put("mr_id", chemistRegistration.getMr_name());
            contentValues.put("route", chemistRegistration.getRouteName());
            contentValues.put("mobile", chemistRegistration.getMobieNo());
            contentValues.put("concernPerson", chemistRegistration.getConcernPerson());
            contentValues.put("address1", chemistRegistration.getAddress());
            contentValues.put("address2", chemistRegistration.getAddress2());
            contentValues.put("address3", chemistRegistration.getAddress3());
            contentValues.put("rout_id", chemistRegistration.getRouteID());
            contentValues.put("pincode", chemistRegistration.getPincode());
            contentValues.put("email", chemistRegistration.getEmail());
            contentValues.put("visitFreequency", chemistRegistration.getVisitFrequency());
            contentValues.put("category", chemistRegistration.getCategory());
            contentValues.put("dateOfBirth", chemistRegistration.getDateOfBirth());
            contentValues.put("marriageAnniversary", chemistRegistration.getMarriageAnniversary());
            sd.insert(PH_Retailer, null, contentValues);
            //return 1;
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
    ...
    //deleate PH_Retailer form table

    public void deleteChRegistration_Table() {
        try {
            sd = this.getWritableDatabase();
            sd.delete(PH_Retailer, null, null);
        } finally {
            sd.close();
        }
    ...
    //bind data on chemist call for Retailer

    public ArrayList<MRetailerAdd> collect_all_ChemistReg_data() {


        ArrayList<MRetailerAdd> ret_Reg = new ArrayList<MRetailerAdd>();
        sd = this.getReadableDatabase();
        //Cursor c=sqLiteDatabase.rawQuery("Select * from PhFarmer", null);
        Cursor c = sd.rawQuery("Select * from " + PH_Retailer, null);
        try {

            if (c.moveToFirst()) {
                do {

                    MRetailerAdd ch = new MRetailerAdd();
                    ch.setId(c.getInt(c.getColumnIndex("id")));
                    ch.setCustmerName(c.getString(c.getColumnIndex("custmerName")));
                    ch.setMobieNo(c.getString(c.getColumnIndex("mobile")));
                    ch.setMr_name(c.getString(c.getColumnIndex("mr_id")));
                    ch.setRouteName(c.getString(c.getColumnIndex("route")));
                    ch.setConcernPerson(c.getString(c.getColumnIndex("concernPerson")));
                    ch.setAddress(c.getString(c.getColumnIndex("address1")));
                    ch.setAddress2(c.getString(c.getColumnIndex("address2")));
                    ch.setAddress3(c.getString(c.getColumnIndex("address3")));
                    ch.setRouteID(c.getString(c.getColumnIndex("rout_id")));
                    ch.setPincode(c.getString(c.getColumnIndex("pincode")));
                    ch.setEmail(c.getString(c.getColumnIndex("email")));
                    ch.setVisitFrequency(c.getString(c.getColumnIndex("visitFreequency")));
                    ch.setCategory(c.getString(c.getColumnIndex("category")));
                    ch.setDateOfBirth(c.getString(c.getColumnIndex("dateOfBirth")));
                    ch.setMarriageAnniversary(c.getString(c.getColumnIndex("marriageAnniversary")));
                    ret_Reg.add(ch);
    ...
    //update tables priyesh

    public void updateRetailerInsertedId(String addID, String insertedId) {
        try {
            sd = this.getWritableDatabase();
            ContentValues cv = new ContentValues();
            cv.put("chem_id", insertedId);
            sd.update("chemisttemp", cv, "chem_id =" + addID, null);
            sd.update("phdcrchem", cv, "chem_id =" + addID, null);
    ...
    public long SaveVisualAdd(MDownLoadVisualAdd mDownLoadVisualAdd) {
        sd = this.getWritableDatabase();
        try {
            ContentValues contentValues = new ContentValues();
            contentValues.put("itemId", mDownLoadVisualAdd.getIteM_ID());
            contentValues.put("itemName", mDownLoadVisualAdd.getIteM_NAME());
            contentValues.put("fileName", mDownLoadVisualAdd.getFilE_NAME());
            contentValues.put("downLoadVersion", mDownLoadVisualAdd.getFilE_DOWNLOAD_VERSION());
            sd.insert(VISUAL_ADD_DOWNLOAD, null, contentValues);
            //return 1;
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
    ...
    //bind data visual add

    public ArrayList<MDownLoadVisualAdd> getListVisualData() {

        ArrayList<MDownLoadVisualAdd> vadList = new ArrayList<MDownLoadVisualAdd>();
        sd = this.getReadableDatabase();
        //Cursor c=sqLiteDatabase.rawQuery("Select * from PhFarmer", null);
        Cursor c = sd.rawQuery("Select * from " + VISUAL_ADD_DOWNLOAD, null);
        try {

            if (c.moveToFirst()) {
                do {

                    MDownLoadVisualAdd vad = new MDownLoadVisualAdd();
                    vad.setID(c.getInt(c.getColumnIndex("id")));
                    vad.setIteM_ID(c.getString(c.getColumnIndex("itemId")));
                    vad.setIteM_NAME(c.getString(c.getColumnIndex("itemName")));
                    vad.setFilE_NAME(c.getString(c.getColumnIndex("fileName")));
                    vad.setFilE_DOWNLOAD_VERSION(c.getString(c.getColumnIndex("downLoadVersion")));
                    vadList.add(vad);
    ...
    public void deleteVisualAddTable() {
        try {
            sd = this.getWritableDatabase();
            sd.delete(VISUAL_ADD_DOWNLOAD, null, null);
        } finally {
            sd.close();
        }
    ...
    public void updateVisualAdd(String versonID, String insertedversonID) {
        try {
            sd = this.getWritableDatabase();
            ContentValues cv = new ContentValues();
            cv.put("itemId", insertedversonID);
            sd.update(VISUAL_ADD_DOWNLOAD, cv, "itemId =" + versonID, null);
