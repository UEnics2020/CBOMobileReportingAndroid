...
import static utils.AppConstant.DR_UPLOAD_DIRECTORY;

public class Doctor_registration_GPS extends AppCompatActivity implements LocationListener,
        GoogleApiClient.ConnectionCallbacks,
        GoogleApiClient.OnConnectionFailedListener, up_down_ftp.AdapterCallback {
    ...
    private CBOImageView attachment1;
    private CboProgressDialog cboProgressDialog;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        attachment1.setTitle("Picture");
        attachment1.setMaxAttachment(1);
        ...
        submit = (Button) findViewById(R.id.add1);
        ...
        attachment1.setListener(new CBOImageView.iCBOImageView() {
            @Override
            public void OnAddClicked() {
                String OutputfileName = Custom_Variables_And_Method.PA_ID + "_" + dr_id + "_drreg_attach_" + Custom_Variables_And_Method.getInstance().get_currentTimeStamp() + ".jpg";
                attachment1.addAttachment((AppCompatActivity) context, AttachImage.ChooseFrom.camera, OutputfileName);

            }

            @Override
            public void OnAdded() {
                OnUpdated(attachment1.getDataList());
            }
            ...
            public void OnUpdated(ArrayList<String> files) {
                ...
                //cbohelp.updateDrFile(Custom_Variables_And_Method.DR_ID, attachment1.getAttachmentStr());
            }
        });
    }
    ...
    private String getAttachmentMsg() {
        String msg = "Please Add " + attachment1.getTitle();
        switch (doc_type) {
            case "D":
                msg = "Please take " + attachment1.getTitle() + " of Doctor Clinic Address";
                break;
            case "C":
                msg = "Please take " + attachment1.getTitle() + " of Chemist Shop Address";
                break;
            case "S":
                msg = "Please take " + attachment1.getTitle() + " of Stockist Office Address";
                break;
    ...
    private void validateAddress() {
        ...
        } else if (attachment1.getAttachmentStr().isEmpty() && attachment1.getVisibility() == View.VISIBLE) {
            //customVariablesAndMethod.msgBox(context, "Please Add " + attachment1.getTitle());
            AppAlert.getInstance().Alert(context, "Alert!!!", getAttachmentMsg(),
                    new View.OnClickListener() {
                        @Override
                        public void onClick(View view) {
                            if (attachment1.getListener() != null) {
                                attachment1.getListener().OnAddClicked();
                            }
                        }
                    });
        } else {
...
//        startCounter();
        if (attachment1.getAttachmentStr().isEmpty() && attachment1.getVisibility() == View.VISIBLE) {
            // customVariablesAndMethod.msgBox(context, "Please Add " + attachment1.getTitle());
            AppAlert.getInstance().Alert(context, "Alert!!!", getAttachmentMsg(),
                    new View.OnClickListener() {
                        @Override
        ...
        cbohelp.updateLatLong(lat_long, dr_id, doc_type, "", attachment1.getAttachmentStr(), "0");
...
//                            T.cancel();
        ...
        } else if (attachment1.filesToUpload().length > 0) {
            cboProgressDialog = new CboProgressDialog(context, "Please Wait..\nuploading Image");
            cboProgressDialog.show();
//            new up_down_ftp().uploadFile(DR_UPLOAD_DIRECTORY, attachment1.filesToUpload(), context);
            new up_down_ftp().uploadToServer(DR_UPLOAD_DIRECTORY, attachment1.filesToUpload(), context);
    ...
    public void complete(Integer responseCode, String message, String description) {
        cboProgressDialog.dismiss();
//        registerDoctorAttachment(lat_long, "");
        registerDoctorAttachment(lat_long, "");
    }
    ...
    private void registerDoctorAttachment(String MOBILE_LAT_LONG, String INDEX) {

        HashMap<String, String> request = new HashMap<>();
        request.put("sCompanyFolder", MyCustumApplication.getInstance().getUser().getCompanyCode());
        request.put("PA_ID", MyCustumApplication.getInstance().getUser().getID());
        request.put("DCS_ARR", doc_type);
        request.put("DCS_ID_ARR", dr_id);
        request.put("DCS_FILE_ARR", attachment1.getAttachmentNameStr());
        request.put("INDEX_ARR", INDEX);
        request.put("LOC_LAT_ARR", MOBILE_LAT_LONG);

        new MyAPIService(context).execute(new ResponseBuilder("DRCHEM_REG_FILECOMMIT", request)
                .setDescription("Please Wait..\n" +
                        "Registration in progress......")
                .setResponse(new CBOServices.APIResponse() {
                    @Override
                    public void onComplete(Bundle bundle) throws Exception {
//                        registerDoctor();
                        cbohelp.updatedLat_Long_RegFile(dr_id);
                        //cbohelp.updateLatLong(MOBILE_LAT_LONG, dr_id, doc_type, INDEX, attachment1.getAttachmentStr(), "1");
                        SyncData();
//                        T.cancel();
                    }
        ...
        switch (reqcode) {
            ...
            case CBOImageView.REQUEST_CAMERA:
                if (rescode == RESULT_OK) {
                    attachment1.onActivityResult(reqcode, rescode, iob);
                }
                break;
            default:
...
//                Toast.makeText(Doctor_registration_GPS.this, "seconds remaining: " + millisUntilFinished / 1000, Toast.LENGTH_SHORT).show();
//            }
//
...
//    private void registerDelaySubmit() {
//        cbohelp.updateLatLong(lat_long, dr_id, doc_type, "", attachment1.getAttachmentStr(), "0");
//        AppAlert.getInstance().Alert(context, "Registered",
//                drname.getText() + "\nRegistration Successfully Completed... ", new View.OnClickListener() {
//                    @Override
