...
public class Spo_Report extends AppCompatActivity {
    ...
    public String DiID, DiName;
    public ProgressDialog progress1;
    Button btShowReport, btBack, btFrom, btTo, btName, fromdatebtn, todatebtn, Display_Unit, HQ_txt, Di_txt;
    ImageView imgFrorm, imgTo, img_followdate, img_nextfollowdate, bt_unit, Di_btn, HQ_btn;
    Context context;
    ...
    ArrayList<SpinnerModel> DiList = new ArrayList<SpinnerModel>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Di_txt = findViewById(R.id.Di_txt);
        Di_btn = findViewById(R.id.Di_btn);
...
//        progress1.show();
        ...
        new MyAPIService(context).execute(new ResponseBuilder("TEAMMONTHDIVISION_MOBILE", request)
                .setTables(tables)
...
//        new CboServices(this,mHandler).customMethodForAllServices(request,"TEAMMONTHDIVISION_MOBILE",MESSAGE_INTERNET,tables);
        ...
        Di_txt.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                setDi();
            }
        ...
        Di_btn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Di_txt.performClick();
            }
...
/*
    ...
    private void setDi() {
        new Spinner_Dialog(context, DiList, new Spinner_Dialog.OnItemClickListener() {
            @Override
            public void ItemSelected(DropDownModel item) {
                DiID = item.getId();
                DiName = item.getName();
                Di_txt.setText(DiName);
                Di_txt.setPadding(1, 0, 5, 0);
            }
        }).show();
    }
...
//    };
    ...
    private void getUnitType(Bundle result) throws JSONException {
        ...
        if (result != null) {
            ...
            String tables1 = result.getString("Tables1");
            JSONArray jsonArray1 = new JSONArray(tables1);
            for (int i = 0; i < jsonArray1.length(); i++) {
                JSONObject c = jsonArray1.getJSONObject(i);
                DiList.add(new SpinnerModel(c.getString("PA_NAME"), c.getString("PA_ID")));
            }
    ...
    private void setData(Bundle bundle) {
        ...
        if (DiList.size() > 0) {
            if (DiList.size() != 1) {
                DiList.add(0, new SpinnerModel("ALL", "0"));
            }
            DiID = DiList.get(0).getId();
            DiName = DiList.get(0).getName();
            Di_txt.setText(DiName);
            Di_txt.setPadding(1, 0, 5, 0);
            //  btTo.setPadding(1, 0, 5, 0);
        }
