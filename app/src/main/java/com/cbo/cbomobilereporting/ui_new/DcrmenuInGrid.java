...
import com.cbo.cbomobilereporting.ui_new.dcr_activities.checkin.CheckIn;
import com.cbo.cbomobilereporting.ui_new.dcr_activities.checkout.Checkout;
...
import utils_new.cboUtils.attachmentsync.AttachmentSync;
import utils_new.cboUtils.attachmentsync.mAttachmentSync;
import utils_new.cboUtils.attachmentsyncdr.AttachmentSyncDr;
...
public class DcrmenuInGrid extends Fragment {
        ...
        switch (menuCodeOnClick) {
            ...
            case "D_DP": {
                if (customVariablesAndMethod.checkIfCallLocationValid(context, true, SkipLocationVarification)) {
                    ...
                    onClickDayPlanning(menuCodeOnClick);
    ...
    private void onClickDayPlanning(String nameOnClick) {
    ...
    //==================================================================dcr planing===========================
    void dcr_plan(String nameOnClick) {
                ...
                .execute(new ResponseBuilder("DCR_DAYPLAN_LOAD_2", request)
                        .setDescription("Please Wait....\nChecking your DCR Status..").setResponse(new CBOServices.APIResponse() {
                            ...
                            public void onComplete(Bundle message) throws JSONException {
                                parser_dcr_plan(message, nameOnClick);
...
//            }
    ...
    private void openDCR(Bundle result, String nameOnClick) {
        ...
        try {
            ...
            if (Custom_Variables_And_Method.DCR_ID.equals("0") || customVariablesAndMethod.getDataFrom_FMCG_PREFRENCE(context, "dcr_date_real").equals("")) {
                ...
                if (nameOnClick.equals("D_INTIME")) {
                    Intent intent = new Intent(getActivity(), CheckIn.class);
                    intent.putExtra("plan_type", "p");
                    startActivity(intent);
                } else if (Custom_Variables_And_Method.ROOT_NEEDED.equals("Y")) {
            ...
            } else if (nameOnClick.equals("D_INTIME")) {
                AppAlert.getInstance().getAlert(context,
    ...
    private Boolean IsCallAllowed(Boolean ckeckForMobileData) {
        ...
        } else if ((customVariablesAndMethod.getDataFrom_FMCG_PREFRENCE(context, "MOBILEDATAYN", "N").equals("Y") || ckeckForMobileData) && !networkUtil.internetConneted(getActivity())) {
            customVariablesAndMethod.Connect_to_Internet_Msg(context);
...
//                        Custom_Variables_And_Method.ROOT_NEEDED = pref.getString("root_needed", null);
    ...
    private void onClickFinalSubmit() {
        ...
        */
        ...
        if (!networkUtil.internetConneted(getActivity())) {
            ...
            ArrayList<mAttachmentSync> arrayList = new ArrayList<>();
            ArrayList<mAttachmentSyncDr> arrayList1 = new ArrayList<>();
            ArrayList<mAttachmentSync> withFile = new ArrayList<mAttachmentSync>();
            ArrayList<mAttachmentSync> withoutFile = new ArrayList<mAttachmentSync>();

            if (MyCustumApplication.getInstance().getDataFrom_FMCG_PREFRENCE("DR_SIGNATURE_MANDATORYYN", "N").equalsIgnoreCase("Y")) {
                arrayList = cboDbHelper.tempDrListFileUpdatePending("0");
            }

            if (MyCustumApplication.getInstance().getDataFrom_FMCG_PREFRENCE("REGBYPICYN", "N").equalsIgnoreCase("Y")) {
                arrayList1 = cboDbHelper.tempDrListAttachedFileUpdatePending("0");
            }
            ...
            if (DCR_ID.equals("0") || customVariablesAndMethod.getDataFrom_FMCG_PREFRENCE(context, "dcr_date_real").equals("")) {
                ...
                ShowAlertforNOCalls();
            } else if (MyCustumApplication.getInstance().getDataFrom_FMCG_PREFRENCE("DR_SIGNATURE_MANDATORYYN", "N").equalsIgnoreCase("Y") && arrayList.size() > 0) {

                ArrayList<mAttachmentSync> finalArrayList = arrayList;

                AppAlert.getInstance().Alert(context, "Signature Pending", "Some Signature are Pending to upload to server......", new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {

                        for (mAttachmentSync attachmentSync : finalArrayList) {

                            if (attachmentSync.getFILE_ATTACHMENT().isEmpty()) {
                                withoutFile.add(attachmentSync);
                            } else {
                                withFile.add(attachmentSync);
                            }
                        }
                        ...
                        if (withoutFile.size() > 0) {
                            ...
                            i.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK);
                            i.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
                            i.putExtra("id", withoutFile.get(0).getID());
                            //i.putExtra("remark","POB entry Pending");
                            startActivity(i);
                        } else {
                            Intent intent = new Intent(context, AttachmentSync.class);
                            intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK);
                            intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
                            intent.putExtra("dataS", withFile);
                            //i.putExtra("remark","POB entry Pending");
                            startActivity(intent);
                        }
            ...
            } else if (MyCustumApplication.getInstance().getDataFrom_FMCG_PREFRENCE("REGBYPICYN", "N").equalsIgnoreCase("Y") && arrayList1.size() > 0) {

                ArrayList<mAttachmentSyncDr> finalArrayList = arrayList1;

                AppAlert.getInstance().Alert(context, "Picture Pending", " Some Pictures are  Pending to upload to server...", new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {

                        Intent intent = new Intent(context, AttachmentSyncDr.class);
                        intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK);
                        intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
                        intent.putExtra("dataS", finalArrayList);
                        //i.putExtra("remark","POB entry Pending");
                        startActivity(intent);
    ...
    private void onClickFinalSubmitCheckOut() {
...
//mandatory_pending_exp_head=cboDbHelper.get_mandatory_pending_exp_head();
        ...
        if (!networkUtil.internetConneted(getActivity())) {
            ...
            if (!DCR_ID.equals("0")) {
                try {
//result4FinalSubmit();
                    PreFinalSubmitCheckINOUT();

                } catch (Exception e) {
                    e.printStackTrace();
                }
            } else {
                customVariablesAndMethod.msgBox(context, "Please Submit Your In-Time First..");
            }
    ...
    public void PreFinalSubmitCheckINOUT() {

        IfDr_Attachment_ConditionFullfilled(new Response() {
            @Override
            public void onSuccess(Bundle bundle) {
    ...
    public void PreFinalSubmit() {
        ...
        IfRetailerConditionFulfiled(new Response() {
            ...
            @Override
            public void onError(String s, String s1) {
                Custom_Variables_And_Method.getInstance().msgBox(context, s1);
//                Toast.makeText(context, , Toast.LENGTH_SHORT).show();
    ...
    private void IfRetailerConditionFulfiled(Response listener) {
        if (MyCustumApplication.getInstance().getDataFrom_FMCG_PREFRENCE("CHEMIST_OFFLINEYN", "N").equalsIgnoreCase("Y")) {
            PreFinalSubmitRetailerForm(listener);
        } else {
            if (listener != null) {
    ...
    private void IfDr_Signature_ConditionFullfilled(Response listener) {

        if (MyCustumApplication.getInstance().getDataFrom_FMCG_PREFRENCE("DR_SIGNATURE_MANDATORYYN", "N").equalsIgnoreCase("Y")) {

//            ArrayList<String> docIds = new ArrayList<>();
            ArrayList<mAttachmentSync> arrayList = new ArrayList<>();
            ArrayList<mAttachmentSync> withFile = new ArrayList<>();
            ArrayList<mAttachmentSync> withoutFile = new ArrayList<>();
            ...
            if (arrayList.size() > 0) {
                ...
                if (listener != null) {
                    listener.onError("Alert !!!", MyCustumApplication.getInstance().getDataFrom_FMCG_PREFRENCE("DR_SIGNATURE_MANDATORYYN", "N"));
                }
            } else {
                if (listener != null) {
                    listener.onSuccess(null);
    ...
    private void IfDr_Attachment_ConditionFullfilled(Response listener) {

        if (MyCustumApplication.getInstance().getDataFrom_FMCG_PREFRENCE("REGBYPICYN", "N").equalsIgnoreCase("Y")) {

            ArrayList<mAttachmentSyncDr> arrayList = new ArrayList<>();

            arrayList = cboDbHelper.tempDrListAttachedFileUpdatePending("0");
//            ArrayList<String> docIds = new ArrayList<>();
//
//            docIds = cboDbHelper.tempDrListAttachedFileUpdatePending("0");
            if (arrayList.size() > 0) {

                ArrayList<mAttachmentSyncDr> finalArrayList = arrayList;
                AppAlert.getInstance().Alert(context, "Picture Pending", " Some Pictures are  Pending to upload to server...", new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
    ...
    private void IfChemistConditionFulfiled(Response listener) {
        ...
        if ((chemist_status == 0) && !MyCustumApplication.getInstance().IsSubmitDCR_WithoutCalls() &&
            ...
            if (listener != null) {
                listener.onError("No " + menuName + "ed !!!", "Please Select Not Visited In " + menuName);
            }
        } else {
            if (listener != null) {
                listener.onSuccess(null);
...
//                            startActivity(i);
    ...
    public void PreFinalSubmitRetailerForm(Response listener) {

        ArrayList<MRetailerAdd> retailerData = new ArrayList<MRetailerAdd>();
        retailerData = cboDbHelper.collect_all_ChemistReg_data();

        StringBuilder sPA_IDStr = new StringBuilder();
        StringBuilder sRETAILER_NAMEStr = new StringBuilder();
        StringBuilder sCONSERN_PERSONStr = new StringBuilder();
        StringBuilder sADDRESS1Str = new StringBuilder();
        StringBuilder sADDRESS2Str = new StringBuilder();
        StringBuilder sADDRESS3Str = new StringBuilder();
        StringBuilder sPINCODEStr = new StringBuilder();
        StringBuilder sEMAILStr = new StringBuilder();
        StringBuilder sCATEGORYStr = new StringBuilder();
        StringBuilder sDOBStr = new StringBuilder();
        StringBuilder sDOAStr = new StringBuilder();
        StringBuilder sMOBILEStr = new StringBuilder();
        StringBuilder sADD_IDStr = new StringBuilder();
        StringBuilder sDISTANCE_IDStr = new StringBuilder();
        StringBuilder sVISIT_FREQStr = new StringBuilder();


        for (MRetailerAdd retailerAdd : retailerData) {
            ...
            sPA_IDStr.append(SEPERATER).append(retailerAdd.getMr_name());
            sRETAILER_NAMEStr.append(SEPERATER).append(retailerAdd.getCustmerName());
            sCONSERN_PERSONStr.append(SEPERATER).append(retailerAdd.getConcernPerson());
            sADDRESS1Str.append(SEPERATER).append(retailerAdd.getAddress());
            sADDRESS2Str.append(SEPERATER).append(retailerAdd.getAddress2());
            sADDRESS3Str.append(SEPERATER).append(retailerAdd.getAddress3());
            sPINCODEStr.append(SEPERATER).append(retailerAdd.getPincode());
            sEMAILStr.append(SEPERATER).append(retailerAdd.getEmail());
            sVISIT_FREQStr.append(SEPERATER).append(retailerAdd.getVisitFrequency());
            sCATEGORYStr.append(SEPERATER).append(retailerAdd.getCategory());
            sDOBStr.append(SEPERATER).append(retailerAdd.getDateOfBirth());
            sDOAStr.append(SEPERATER).append(retailerAdd.getMarriageAnniversary());
            sDISTANCE_IDStr.append(SEPERATER).append(retailerAdd.getRouteID());
            sMOBILEStr.append(SEPERATER).append(retailerAdd.getMobieNo());
            sADD_IDStr.append(SEPERATER).append((retailerAdd.getId() * -1));
        ...
        request.put("sPA_ID", sPA_IDStr.toString());
        request.put("sRETAILER_NAME", sRETAILER_NAMEStr.toString());
        request.put("sCONSERN_PERSON", sCONSERN_PERSONStr.toString());
        request.put("sADDRESS1", sADDRESS1Str.toString());
        request.put("sADDRESS2", sADDRESS2Str.toString());
        request.put("sADDRESS3", sADDRESS3Str.toString());
        request.put("sPINCODE", sPINCODEStr.toString());
        request.put("sEMAIL", sEMAILStr.toString());
        request.put("sVISIT_FREQ", sVISIT_FREQStr.toString());
        request.put("sCATEGORY", sCATEGORYStr.toString());
        request.put("sDOB", sDOBStr.toString());
        request.put("sDOA", sDOAStr.toString());
        request.put("sDISTANCE_ID", sDISTANCE_IDStr.toString());
        request.put("sMOBILE", sMOBILEStr.toString());
        request.put("sADD_ID", sADD_IDStr.toString());


        ArrayList<Integer> tables = new ArrayList<>();
        tables.add(0);

        new MyAPIService(context).execute(new ResponseBuilder("CHEMIST_COMMIT_MOBILE", request)
                .setDescription("Please Wait..\n" +
                        "Retailer add in progress......")
                .setTables(tables)
...
//                        parseJson(bundle, context, listener);
    ...
    private void parseJson(Bundle bundle, Context context, Response listener) throws Exception {
        CBO_DB_Helper cboHelper = new CBO_DB_Helper(context);
        ...
        for (int i = 0; i < jsonArray1.length(); i++) {
            ...
            String insertedId = object.getString("INSERTED_ID");
            cboHelper.updateRetailerInsertedId(addId, insertedId);
        }

        MyCustumApplication.getInstance().setDataInTo_FMCG_PREFRENCE("RETAILERADD_ROUT_ID", "");
        MyCustumApplication.getInstance().setDataInTo_FMCG_PREFRENCE("RETAILERADD_ROUT_NAME", "");
        MyCustumApplication.getInstance().setDataInTo_FMCG_PREFRENCE("RETAILERADD_PA_ID_L", "");
        MyCustumApplication.getInstance().setDataInTo_FMCG_PREFRENCE("RETAILERADD_CAT_ID", "");
        MyCustumApplication.getInstance().setDataInTo_FMCG_PREFRENCE("RETAILERADD_CAT_NAME", "");
        MyCustumApplication.getInstance().setDataInTo_FMCG_PREFRENCE("RETAILERADD_MR_ID", "");
        MyCustumApplication.getInstance().setDataInTo_FMCG_PREFRENCE("RETAILERADD_MR_NAME", "");
        cboHelper.deleteChRegistration_Table();
    ...
    private boolean checkVisualCall() {
        if (Custom_Variables_And_Method.getInstance().isVisualAddDownloadRequired(context)) {
            return true;
        } else {
            return false;
