...
import com.cbo.cbomobilereporting.ui_new.utilities_activities.newDocPhoto.New_Show_Sample;
...
public class DrCall extends AppCompatActivity implements ExpandableListAdapter.Summary_interface {
    ...
    private static final int GPS_TIMMER = 4, MESSAGE_INTERNET_DCRCOMMIT_DOWNLOADALL = 1, WORK_WITH_DIALOG = 0, MESSAGE_INTERNET_SEND_FCM = 2, MESSAGE_FROM_CHECK_VISUAL_AD = 7;
    ...
    private ArrayList<String> selected_group_item_ids = new ArrayList<>();
    private ArrayList<String> selected_group_item_names = new ArrayList<>();
    ...
    public void onCreate(Bundle b) {
        ...
        MyCustumApplication.getInstance().setDataInTo_FMCG_PREFRENCE("SAVE", "");

        if (customVariablesAndMethod.getDataFrom_FMCG_PREFRENCE(context, "DCRSAMPLE_AFTERVISUALAIDYN").equalsIgnoreCase("Y")) {
            submit.setText("Visual Ads");
        } else {
            submit.setText("Submit Call");
        }
...
//        showRegistrtion = Integer.parseInt(customVariablesAndMethod.getDataFrom_FMCG_PREFRENCE(context, "IsBackDate", "1"));
        showRegistrtion = customVariablesAndMethod.isGeoFencingRequired(CallType.DOCTOR) ? 1 : 0;
...
//                }
                ...
                } else if (customVariablesAndMethod.getDataFrom_FMCG_PREFRENCE(context, "DCRSAMPLE_AFTERVISUALAIDYN").equalsIgnoreCase("Y")) {
                    pressedVisualAd();
...
//                    String planTime = customVariablesAndMethod.getDataFrom_FMCG_PREFRENCE(context,"DcrPlantimestamp",customVariablesAndMethod.get_currentTimeStamp());
                    String planTime = customVariablesAndMethod.getDataFrom_FMCG_PREFRENCE(context, "LastCallTime", customVariablesAndMethod.get_currentTimeStamp());
...
//        }
        ...
        if (!MyCustumApplication.getInstance().getDataFrom_FMCG_PREFRENCE("DCRSAMPLE_AFTERVISUALAIDYN", "N").equalsIgnoreCase("Y")) {
            ...
            AppAlert.getInstance().Alert(context, "Alert !!", doc_name + " Added Successfully", new View.OnClickListener() {
                @Override
                public void onClick(View view) {
//                            finish();
                    new Service_Call_From_Multiple_Classes().SendFCMOnCall(context, mHandler, MESSAGE_INTERNET_SEND_FCM, "D", dr_id, latLong);
    ...
    public void submitDoctor(boolean Skip_Verification) {
        ...
        } else if (!Skip_Verification && MyCustumApplication.getInstance().getDataFrom_FMCG_PREFRENCE("DCRSAMPLE_AFTERVISUALAIDYN", "N") == "Y") {
            pressedVisualAd();

        } else if (!Skip_Verification && !customVariablesAndMethod.checkIfCallLocationValid(context, false, Skip_Verification)) {
...
/*
    ...
    private void pressedVisualAd() {
        MyCustumApplication.getInstance().setDataInTo_FMCG_PREFRENCE("utility", "");
        Intent intent = new Intent(DrCall.this, NewDocPhoto.class);
        intent.putExtra("dr_id", dr_id);
        startActivityForResult(intent, MESSAGE_FROM_CHECK_VISUAL_AD);
    }
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MESSAGE_INTERNET_SEND_FCM:
                    ...
                    if (!call_missed.isChecked() && customVariablesAndMethod.IsProductEntryReq(context, CallType.DOCTOR)) {
                        Intent i = new Intent(getApplicationContext(), Doctor_Sample.class);
                        i.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK);
                        i.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
                        i.putExtra("id", dr_id);
                        i.putExtra("dr_name", dr_name);
                        startActivity(i);
                    }
                    finish();
                    break;
...
//                    Toast.makeText(DrCall.this, "hello", Toast.LENGTH_SHORT).show();
//                    Bundle b3 = msg.getData();
//                    selected_group_item_ids = b3.getStringArrayList("itemId");
//                    selected_group_item_names = b3.getStringArrayList("itemName");
...
//                    }
        ...
        listview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (!model.getAPP_PENDING_YN().equalsIgnoreCase("0")) {
                    ...
                    alertdFragment.setAddressRequired(MyCustumApplication.getInstance().getDataFrom_FMCG_PREFRENCE("DCRDRREG_RESETYN", "N").equalsIgnoreCase("Y"));
                    alertdFragment.setResetRequired(MyCustumApplication.getInstance().getDataFrom_FMCG_PREFRENCE("DCRDRREG_RESETYN", "N").equalsIgnoreCase("Y"));


                    alertdFragment.setCallDetail(dr_id, "D", new OnClickListener() {
                        @Override
                        public void onClick(View view) {
                            cbohelp.updateLatLong("", dr_id_reg, "D", "");
                            cbohelp.updateLatLong("", dr_id_reg, "D", "2");
                            cbohelp.updateLatLong("", dr_id_reg, "D", "3");
                        }
                    });
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        switch (requestCode) {
            ...
            case 7:
                if (MyCustumApplication.getInstance().getDataFrom_FMCG_PREFRENCE("SAVE", "").equalsIgnoreCase("SAVEDATA")) {
                    String data1 = MyCustumApplication.getInstance().getDataFrom_FMCG_PREFRENCE("DATA", "");
                    submitDoctor(true);
                    MyCustumApplication.getInstance().setDataInTo_FMCG_PREFRENCE("SAVE", "");
                }
