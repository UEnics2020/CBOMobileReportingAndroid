...
public class DashboardDB extends DBHelper {
    ...
    public String getTableQuery() {
        return "CREATE TABLE " + getTable() +
                ...
                "CATEGORY text,REMARK text,AMOUNT text," +
                "AMOUNT_CUMM text,URL text,BG_COLOR text)";
    }
    ...
    @Override
    public void onTableUpdate(SQLiteDatabase sqLiteDatabase, int oldVersion, int newVersion) {
        switch (oldVersion){
            case 1:
                sqLiteDatabase.execSQL("ALTER TABLE " + this.getTable() + " ADD COLUMN BG_COLOR text DEFAULT ''");
        }
    }
    ...
    public void insert(Map<String, String> dashboard,String Category){
        ...
        contentValues.put("BG_COLOR", dashboard.get("BG_COLOR"));
        ...
        try {
            if (c.moveToFirst()) {
                do {
                    ...
                    datanum1.put("BG_COLOR",  c.getString(c.getColumnIndex("BG_COLOR")));
                    data.add(datanum1);
