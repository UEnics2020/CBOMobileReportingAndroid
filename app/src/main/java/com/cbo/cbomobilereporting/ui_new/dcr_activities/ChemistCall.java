...
//        showRegistrtion = Integer.parseInt(customVariablesAndMethod.getDataFrom_FMCG_PREFRENCE(context, "IsBackDate", "1"));
        ...
        showRegistrtion = customVariablesAndMethod.isGeoFencingRequired(CallType.CHEMIST) ? 1 : 0;
        ...
        if (customVariablesAndMethod.IsProductEntryReq(context, CallType.CHEMIST)) {
...
//                String planTime = customVariablesAndMethod.getDataFrom_FMCG_PREFRENCE(context,"DcrPlantimestamp",customVariablesAndMethod.get_currentTimeStamp());
    ...
    class ChemistData extends AsyncTask<ArrayList<SpinnerModel>, String, ArrayList<SpinnerModel>> {
        ...
        protected ArrayList<SpinnerModel> doInBackground(ArrayList<SpinnerModel>... params) {
            ...
            try {
                ...
                //priyesh
                ArrayList<MRetailerAdd> retData = cbohelp.collect_all_ChemistReg_data();
                for (int i = 0; i < retData.size(); i++) {
                    if (MyCustumApplication.getInstance().getDataFrom_FMCG_PREFRENCE("ROUTE_ID", "").equalsIgnoreCase(retData.get(i).getRouteID().trim())) {
                        chemist.add(new SpinnerModel(retData.get(i).getCustmerName(), "" + retData.get(i).getId() * -1, retData.get(i).getVisitFrequency(), "", "", "", ""));
                    }
                }
            } catch (Exception e) {
            }
    ...
    public void submitChemist(boolean Skip_Verification) {
        ...
        } else if (Custom_Variables_And_Method.getInstance().isCheckForProduct("C")) {
            if (!(PobAmt.equals("") || PobAmt.equals("0")) && (sample_name.equals("")) && customVariablesAndMethod.IsProductEntryReq(context, CallType.CHEMIST)) {
                ...
                if (cbohelp.searchChemist(chm_id).contains(chm_id)) {
                    ...
                    try {
                        ...
                        if (chm_ok.equals("")) {
                            ...
                            if (Hide_status.equals("N")) {
                                customVariablesAndMethod.msgBox(context, "Chemist Call Add Successfully");
                            } else {
                                customVariablesAndMethod.msgBox(context, "Retailer Call Add Successfully");
                            }
    ...
    public void Edit_Call(final String Dr_id, final String Dr_name) {
        ...
        if (PobMailDb.getInstance(context).getChemist(new mChemist().setID(Integer.parseInt(Dr_id))).size() > 0) {
            AppAlert.getInstance().getAlert(context, "Alert!!!", "You are not allowed to Edit " + Dr_name + "...\n" +
                    "Mail has already been send...");
        } else {
            AppAlert.getInstance()
                    .setNagativeTxt("Cancel")
                    .setPositiveTxt("Edit")
                    .DecisionAlert(context, "Edit!!!", "Do you want to edit " + Dr_name + " ?",
                            new AppAlert.OnClickListener() {
                                @Override
                                public void onPositiveClicked(View item, String result) {
                                    call_layout.setVisibility(View.VISIBLE);
    ...
    public void delete_Call(final String Dr_id, final String Dr_name) {
        ...
        if (PobMailDb.getInstance(context).getChemist(new mChemist().setID(Integer.parseInt(Dr_id))).size() > 0) {
            AppAlert.getInstance().getAlert(context, "Alert!!!", "You are not allowed to Delete " + Dr_name + "...\n" +
                    "Mail has already been send...");
        } else {

            AppAlert.getInstance()
                    .setPositiveTxt("Delete")
                    .DecisionAlert(context, "Delete!!!", "Do you Really want to delete " + Dr_name + " ?",
                            new AppAlert.OnClickListener() {
                                @Override
                                public void onPositiveClicked(View item, String result) {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (((TextView) view.findViewById(R.id.distance)).getText().toString().equals("Registration pending...")) {
                    ...
                    if (cbohelp.searchChemist(chm_id).contains(chm_id)) {
                        ...
                        stk.removeAllViews();
                        gift_layout.removeAllViews();
                        save.setText("Submit Call");
                        retail_save.setText("Submit Call");
                    }
