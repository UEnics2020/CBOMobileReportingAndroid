...
//        showRegistrtion = Integer.parseInt(customVariablesAndMethod.getDataFrom_FMCG_PREFRENCE(context, "IsBackDate", "1"));
        showRegistrtion = customVariablesAndMethod.isGeoFencingRequired(CallType.STOKIST) ? 1 : 0;
        ...
        //head change dynamic
        headTitle = cbohelp.getMenu("DCR", "D_STK_CALL").get("D_STK_CALL").split(" ")[0];
        hader_text.setText(headTitle + " Call");
        tvCapName.setText(headTitle + " Name");
        text_view_Regular.setText(headTitle + " Not Visited");
...
//                            "Are you sure to save as\n\"No " + "Stockist" + " for the day",
        ...
        if (customVariablesAndMethod.IsProductEntryReq(context, CallType.STOKIST)) {
...
//                String planTime = customVariablesAndMethod.getDataFrom_FMCG_PREFRENCE(context,"DcrPlantimestamp",customVariablesAndMethod.get_currentTimeStamp());
        ...
        summary_list.put(cbohelp.getMenu("DCR", "D_STK_CALL").get("D_STK_CALL").split(" ")[0], stockist_list_summary);
...
//        listAdapter = new ExpandableListAdapter(summary_layout, this, header_title, summary_list);
   ...
   /* protected void onActivityResult(int reqcode, int rescode, Intent iob) {
    ...
    public void submitStockist(boolean Skip_Verification) {
        ...
        } else if (Custom_Variables_And_Method.getInstance().isCheckForProduct("S")) {

            if (!(PobAtm1.equals("") ||
                    PobAtm1.equals("0")) &&
                    (sample_name.equals("")) &&
                    customVariablesAndMethod.IsProductEntryReq(context, CallType.STOKIST)) {
...
//                            customVariablesAndMethod.msgBox(context, "Stockist Added Successfully");
                            customVariablesAndMethod.msgBox(context, headTitle + " Added Successfully");
                            pob.setText("");
            ...
            if (cbohelp.searchStockist(stk_id).contains(stk_id)) {
                ...
                Log.e("stockist updated", "" + val);
//                customVariablesAndMethod.msgBox(context, stkst_name + "  successfully updated");
                customVariablesAndMethod.msgBox(context, headTitle + "  successfully updated");
...
//        new GPS_Timmer_Dialog(context, mHandler, "Scanning Stockist...", GPS_TIMMER).show();
        new GPS_Timmer_Dialog(context, mHandler, "Scanning " + headTitle + "...", GPS_TIMMER).show();
    }
    ...
    class StokistData extends AsyncTask<ArrayList<SpinnerModel>, String, ArrayList<SpinnerModel>> {
        ...
        protected void onPostExecute(ArrayList<SpinnerModel> s) {
            ...
            if ((!s.isEmpty()) || (s.size() < 0)) {
                ...
                customVariablesAndMethod.getAlert(context, headTitle + " not Found", "No  " + headTitle + " In Planed Dcr..");
