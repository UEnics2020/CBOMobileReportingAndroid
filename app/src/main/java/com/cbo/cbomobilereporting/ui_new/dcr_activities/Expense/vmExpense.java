...
public class vmExpense extends CBOViewModel<IExpense> {
    ...
    public void setSelectedDistance(mDistance distance) {
        ...
        if ((getExpense().getSelectedDA().getCode().contains("EX") ||
                getExpense().getSelectedDA().getCode().contains("NSD"))
                && !getExpense().getDARATE_SLAB_DOUBLESIDE().equalsIgnoreCase("Y")) {
            getExpense().getSelectedDA().setTA_Rate(getExpense().getRateFor(getExpense().getSelectedDA().getTA_Km() / 2).getRate());
        } else {
...
//                            jsonObject1.getString("MAX_AMT"), jsonObject1.getString("TAMST_VALIDATEYN"));
        ...
        for (int i = 0; i < jsonArray3.length(); i++) {
            ...
            MyCustumApplication.getInstance().setDataInTo_FMCG_PREFRENCE("PER_KM", "" + object.getDouble("FARE_RATE_NEW"));
...
//                            "" + othExpense.getId(),othExpense.getTime());
                        ...
                        if (value2.equalsIgnoreCase("1")) {
                            if (IsFormForFinalSubmit()) {
                                ...
                                if (MyCustumApplication.getInstance().getDataFrom_FMCG_PREFRENCE("EXPTA_MGRMANUALYN", "Y").equalsIgnoreCase("Y") && !customVariablesAndMethod.getDataFrom_FMCG_PREFRENCE(context, "DA_TYPE", "").equalsIgnoreCase("L")) {
                                    Custom_Variables_And_Method.getInstance().setDataInTo_FMCG_PREFRENCE(context, "ACTUALFARE", "" + MyCustumApplication.getInstance().getDataFrom_FMCG_PREFRENCE("ACTUALFARE", ""));
                                } else {
    ...
    private void setLastUpdateData(Bundle bundle) throws JSONException {
        ...
        for (int i = 0; i < jsonArray3.length(); i++) {
            object = jsonArray3.getJSONObject(i);
            distanceId = object.getString("DCR_DISTANCE_ID");
            selectedDA = object.getString("DCR_L_EX_NS");
        }


        String table3 = bundle.getString("Tables3");
        JSONArray jsonArray4 = new JSONArray(table3);
        for (int i = 0; i < jsonArray4.length(); i++) {
            JSONObject object1 = jsonArray4.getJSONObject(i);
            mDA da = new mDA();
            ...
            if (selectedDA.equalsIgnoreCase(da.getCode())){
                setSelectedDA(da);
                break;
            }
        ...
        if (Integer.parseInt(distanceId) > 0) {
            String table4 = bundle.getString("Tables4");
            JSONArray jsonArray5 = new JSONArray(table4);
            for (int i = 0; i < jsonArray5.length(); i++) {
                JSONObject object1 = jsonArray5.getJSONObject(i);
                mDistance distance = new mDistance();
                ...
                distance.setMANUAL_TAYN(object1.getString("MANUAL_TAYN"));
                distance.setMANUAL_TAYN_MANDATORY(object1.getString("MANUAL_TAYN_MANDATORY"));
                if (distanceId.equalsIgnoreCase(object1.getString("DISTANCE_ID"))) {
