...
public class Dcr_Workwith_Adapter extends ArrayAdapter<Dcr_Workwith_Model>
	{
		...
		public Dcr_Workwith_Adapter(AppCompatActivity context, ArrayList<Dcr_Workwith_Model> list, String[] selected_list, String[] independent_list)
		{
			...
			setSelected_list(list,selected_list);

			if (!Custom_Variables_And_Method.pub_desig_id.equals(""+1)) {
				show_independent = true;
				setIndependent_list(list,independent_list);
			}else{
		...
		private void setIndependent_list(ArrayList<Dcr_Workwith_Model> list, String[] independent_list){

			ArrayList<String > selected_array = new ArrayList<String >(Arrays.asList(independent_list));
			for (Dcr_Workwith_Model model : list){
				if (selected_array.contains(model.getName())){
					model.setindependentSelected(true);
				}
			}
		}

		private void setSelected_list(ArrayList<Dcr_Workwith_Model> list, String[] selected_list) {

			ArrayList<String> selected_array = new ArrayList<String>(Arrays.asList(selected_list));
			for (Dcr_Workwith_Model model : list) {
				if (selected_array.contains(model.getName())) {
