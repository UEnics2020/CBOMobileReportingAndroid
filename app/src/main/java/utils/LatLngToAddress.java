...
//                URL url = new URL("https://maps.googleapis.com/maps/api/geocode/json?latlng="+ Uri.encode(address.getLAT_LONG())+"&key=AIzaSyB4UzWR8_mncvSPW_CyA485WryqI6Z4D20");
                URL url = new URL("https://maps.googleapis.com/maps/api/geocode/json?latlng=" + Uri.encode(address.getLAT_LONG()) + "&key=" + MyCustumApplication.getInstance().getDataFrom_FMCG_PREFRENCE("GOOGLEMAPAPI_KEY", ""));
