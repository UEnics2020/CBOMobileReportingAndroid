...
//                URL url = new URL("https://maps.googleapis.com/maps/api/geocode/json?address="+ Uri.encode(params[0])+"&key=AIzaSyB4UzWR8_mncvSPW_CyA485WryqI6Z4D20");
                URL url = new URL("https://maps.googleapis.com/maps/api/geocode/json?address=" + Uri.encode(params[0]) + "&key=" + MyCustumApplication.getInstance().getDataFrom_FMCG_PREFRENCE("GOOGLEMAPAPI_KEY", ""));
