        ...
        public Map<String,String> dcr_doctorSave(String updated)
        {
            ...
            if(mydrList.size()>0) {
                for (int i = 0; i < mydrList.size(); i++) {
                    try {
                        String dr_latLong, dr_Address,dr_locExtra,file="";
                    ...
                    if (i==0){
                        ...
                        file = cbohelp.getFILE_Doctor(mydrList.get(i));
                        file = file.substring(file.lastIndexOf("/")+1);
                        sb_sDCRDR_FILE.append(saprator).append(file);
            ...
            if(chem_id.size()>0) {
               String saprator="";
                ...
                for (int i = 0; i < chem_id.size(); i++) {
                        ...
                        try {
                            String chemLatLong, chemAddress, chem_locExtra,file="";
                        ...
                        sb_sDCRCHEM_REMARK.append(saprator).append( cbohelp.getRemark_Chemist(chem_id.get(i)));
                            file = cbohelp.getFileChemist(chem_id.get(i));
                            file = file.substring(file.lastIndexOf("/")+1);
                        sb_sDCRCHEM_FILE.append(saprator).append( file);
            ...
            if(stk_id.size()>0) {
                for (int i = 0; i < stk_id.size(); i++) {
                    ...
                    try {
                        ...
                        file = cbohelp.getFile_Stockist(stk_id.get(i));
                        file = file.substring(file.lastIndexOf("/")+1);
                        sb_sDCRSTK_FILE.append(saprator).append(file);
            ...
            if(dr_remId.size()>0) {
                ...
                for (int i = 0; i < dr_remId.size(); i++) {
                    try {
                        ...
                        file = cbohelp.Rc_file(dr_remId.get(i));
                        file = file.substring(file.lastIndexOf("/")+1);
                        sb_sDCRRC_FILE.append(saperator).append(file);
        ...
        if(chem_remId.size()>0) {
            ...
            for (int i = 0; i < chem_remId.size(); i++) {
                try {
                    ...
                    file = cbohelp.ChemRc_file(chem_remId.get(i));
                    file = file.substring(file.lastIndexOf("/")+1);
                    sb_sDCRRC_REMARK_CHEM.append(saperator).append(cbohelp.ChemRc_remark(chem_remId.get(i)));
                    sb_sDCRRC_FILE_CHEM.append(saperator).append(file);
        ...
        if(phdairy_dcr.size()>0) {
            ...
            for (int i = 0; i < phdairy_dcr.size(); i++) {
                try {
                    ...
                    file = phdairy_dcr.get(i).get("file");
                    file = file.substring(file.lastIndexOf("/")+1);
                    sb_sDAIRY_FILE.append(saperator).append(file);
